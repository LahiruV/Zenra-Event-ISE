            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {!isAuthPage && (
                <>
                  ...
                  <Link to="/feedback" className={`block px-3 py-2 ${isActive('/feedback')}`}>
                    Feedback
                  </Link>
                  <Link to="/chatbot" className={`block px-3 py-2 ${isActive('/chatbot')}`}>
                    Chat
                  </Link>
                  <Link to="/inquiry" className={`block px-3 py-2 ${isActive('/inquiry')}`}>
                    Inquiry
                  </Link>
                </>
              )}
              {!isAuthenticated ? (
                ...
                <button
                  onClick={logout}
                  className="block px-3 py-2 text-gray-700 hover:text-yellow-500 w-full text-left flex items-center space-x-1"
                >
                  <LogOut className="h-4 w-4" />
                  <span>Logout</span>
                </button>
